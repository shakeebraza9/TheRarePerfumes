...
class OrdersExport implements FromView
{
    ...
    {
        $query = Order::with('children'); // eager load order items
        ...
        // ✅ Add comma-separated product names for each order
        foreach ($orders as $order) {
            $order->product_names = $order->orderItems
                ->pluck('title')       // get all titles
                ->map(fn($t) => trim($t)) // remove extra spaces

...
public function home()
{
    ...
    // Featured products (category + brand enable)
    ...
    // Collection products (category + brand enable)
...
public function shop(Request $request)
{
    ...
    if ($request->filled('brandid')) {
        $data->where('brand_id', $request->brandid);
    }

    // Collection filter
    if ($request->filled('collection')) {
        $collection = Collection::where('slug', $request->collection)->first();
        if (!$collection) return back();

        $productIds = ProductCollection::where('collection_id', $collection->id)
            ->pluck('product_id');
    ...
    // Pagination (IMPORTANT: yahin get hota hai)
    ...
    return view('theme.shop.shop', compact(
        'data', 'categories', 'collections',
        'col', 'cat', 'discountPercent'
    ));
}
...
public function product($id)
{
    ...
    // ❌ Product / category / brand off → home redirect
    if (!$product) {
        return redirect('/')->with('error', 'Product not available');
    }

    // Related products (only enabled ones)
    $releated_products = Product::query()
        ->where('is_enable', 1)
        ->whereHas('category', fn ($q) => $q->where('is_enable', 1))
        ->whereHas('brand', fn ($q) => $q->where('is_enable', 1))
        ->inRandomOrder()
        ->limit(5)
        ->get();
    ...
    // Discount
    $discountSetting = DB::table('settings')->where('field', 'discount_percent')->first();
    $discount = ($discountSetting && is_numeric($discountSetting->value))
        ? (float) $discountSetting->value
        : 0;
    ...
    foreach ($product->variations as $variation) {
        foreach ($variation->attributes as $attribute) {
            ...
            if ($discount > 0 && $variationPrice > 0) {
                $variationPrice = round(
                    $variationPrice - (($variationPrice * $discount) / 100),
                    2
                );
            }

...
public function index(Request $request, $id)
{
    try {
        ...
        return back()->with('warning', 'Invalid ID');
    }

    // Fetch the related attribute (variation)
    $variation = Attribute::find($decryptedId);

    // Fetch the first Value where attribute_id matches
    $value = Value::where('attribute_id', $decryptedId)->first();

    // If no Value found, initialize an empty Value with attribute_id
    if (!$value) {
        $value = new Value();
        $value->attribute_id = $decryptedId;
    }

    // Always fetch all Values linked to this attribute
    $pageItems = Value::where('attribute_id', $decryptedId)->get();
